...
public class CommandFill extends MaterialCommand implements ICommand {
	...
	public void process(DigitalClock main, Player player, String[] args) {
		...
		if(super.isUsableName(args[2])) {
			...
			if(super.isSolid(m)) {
				player.sendMessage(ChatColor.DARK_GREEN + DigitalClock.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed filling material from " + oldmat + " to "+ clock.setFillingMaterial(m).name().toLowerCase().replace("_", " "));
			} else {
